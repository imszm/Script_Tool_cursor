...
class AutoTester:
    def __init__(self, ble: BLEProtocol, out_dir: str):
        self.ble = ble
    ...
    def load_cases(self, path: str) -> List[Dict[str, Any]]:
        ...
        if _HAS_PANDAS:
            ...
            if ext in ('.xlsx', '.xls'):
                ...
                raise ValueError("仅支持 xlsx/xls/csv 用例")
            records = df.to_dict(orient='records')
            return records
        else:
            # 退化：仅支持 CSV
            if not path.lower().endswith('.csv'):
                raise ValueError("未安装 pandas/openpyxl 时仅支持 CSV 用例，请提供 .csv 文件")
            import csv
    ...
    def run_cases(self, cases: List[Dict[str, Any]]):
        for idx, row in enumerate(cases, start=1):
            ...
            sent_hex = None  # （如需，亦可在 format 时捕获原始发送数据）
            ...
            # 断言：有提供 expect_* 字段则对比
            verdict, checks = self._assert_led_expectations(row, led_msg if read_ok else None)
...
def main():
    ...
    # 如果用户没有任何参数（直接点 Run），就默认开启 combo 模式
    if len(sys.argv) == 1:
        args.combo = False    
    ...
    try:
        ...
        if args.loop_count > 0 and len(cases) > 0:
            tester.loop_case(cases[0], loop_count=args.loop_count, delay=args.loop_delay)
    ...
    encoding = detect_encoding('testcases.csv')
    with open('testcases.csv', 'r', encoding=encoding) as f:
        reader = csv.reader(f)
