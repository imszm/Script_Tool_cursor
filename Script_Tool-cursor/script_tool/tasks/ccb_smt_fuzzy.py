...
from script_tool.drivers.vision import detect_pass_fail_fuzzy
...
@dataclass
class CcbSmtFuzzyTask:
    name: str = "ccb-smt-fuzzy"
    ...
    def run(self, ctx: RunContext) -> int:
        ...
        try:
            ...
            ctx.logger.error(f"Connect failed: {e}")
            return 2
        ...
        for i in range(loops):
            ...
            try:
                edit = dlg.child_window(auto_id="Widget.lineEditSerialNumber", control_type="Edit")
                edit.set_edit_text(sn)
                dlg.type_keys("{ENTER}")
            except Exception:
                ctx.logger.exception("Input SN failed")
                continue
            ...
            if res == "FAIL":
                out = ctx.paths.artifacts_dir / f"ccb_fuzzy_fail_{sn}.png"
                screenshot(dlg, out)
                return 3
