...
class BLEProtocol:
    ...
    def parse_data(self, data: bytes) -> Tuple[bool, Optional[ppx_ble_msg_t], Optional[int], Optional[int]]:
        """ 用 DLL 的解析函数解析数据包，并返回 DLL 的原始返回码（result） """
        if not data:
        ...
        try:
            ...
            # 保存 DLL 返回值
            parse_result = self.ble_lib.ppx_com_ble_parse(
            ...
            if parse_result == PPX_PARSE_SUCCESS:
                ...
                return True, ble_msg, cmd_type, int(parse_result)
            else:
        ...
        try:
            ...
            if success and data is not None:
                if self.send_data(data):
                    ...
                    if response:
                        # 现在 parse_data 返回第四项为 parse_result
    ...
    def read_led_status(self, recv_timeout: Optional[float] = None) -> Tuple[bool, Optional[bytes], Optional[ppx_led_msg_t], Optional[int]]:
        if not self.dll_loaded:
        ...
        try:
            ...
            if success and data is not None:
                if self.send_data(data):
                    ...
                    if response:
                        parse_success, parsed_msg, cmd_type, parse_result = self.parse_data(response)
                        return parse_success, response, self.g_ppx_ble_data.led_msg if parse_success else None, parse_result
            return False, None, None, None
        except Exception as e:
...
class AutoTester:
    ...
    def run_cases(self, cases: List[Dict[str, Any]]):
        for idx, row in enumerate(cases.to_dict(orient="records"), start=1):
            ...
            # 写入命令
            ok, resp, parse_send = self.ble.set_led_display(**params, recv_timeout=recv_timeout)
            recv_hex = self.ble._bytes_to_hex(resp) if ok and resp else ''

            # 读取响应
            read_ok, read_resp, led_msg, parse_read = self.ble.read_led_status(recv_timeout=recv_timeout)
    ...
    def loop_case(self, case: Dict[str, Any], loop_count: int = 100, delay: float = 0.5):
        ...
        for i in range(1, loop_count + 1):
            result = self.ble.set_led_display(**params)  # 只调用一次
            print("DEBUG 返回值:", result)  # 打印原始返回
            ok = bool(result)  # 如果 result 能转成布尔，作为成功失败判断
    ...
    def save_results_csv(self, path: str):
        ...
        keys = [
            ...
            'recv_timeout','delay_after','send_ok','read_ok','recv_hex','read_hex','parse_send','parse_read','verdict','checks','elapsed_s','timestamp'
        ]
    ...
    def save_report_html(self, path: str, title: str = "BLE 自动化测试报告"):
        ...
        for r in self.results:
            rows_html.append(
                ...
                # === 保持数据一致 ===
                f"<td style='font-family:monospace'>{esc(r.get('recv_hex'))}</td>",   # 下行数据(Write)
                f"<td style='font-family:monospace'>{esc(r.get('read_hex'))}</td>",   # 上行数据(Read)
                f"<td>{esc(r.get('parse_send'))}</td>",  # Write解析结果
                f"<td>{esc(r.get('parse_read'))}</td>",  # Read解析结果
                # === verdict 只保留一次 ===
...
def make_combo_cases():
    ...
    for values in itertools.product(*[fields[k] for k in keys]):
        ...
        # ⭐ 过滤条件：只保留 screen_on=1 且 brightness >= 7
